// --- 1. НАШ КЛЮЧ MAPILLARY И ГОРОДА С РЕГИОНАМИ ---
const myAccessToken = 'MLY|26266369579645993|cf11c635af876bc5f1a671e0f6401897';

// Добавили поле region каждому городу
const CITIES = [
    // Европа и СНГ
    ...
    { name: "Тель-Авив", lat: 32.0853, lon: 34.7818, region: "asia" }, 
    // Африка и Океания
...
// Глобальные настройки игры (по умолчанию)
let gameSettings = {
    region: 'all',
    time: 120 // в секундах
};
...
async function getRandomImageId() {
    ...
    // Фильтруем массив городов перед выбором случайного
...
function updateTimerDisplay() {
    if (gameSettings.time === 0) return; // Если без времени, не обновляем текст
...
function startTimer() {
    ...
    // Проверяем режим "Без таймера"
    if (gameSettings.time === 0) {
        timerDisplay.innerText = "∞";
        return; // Выходим из функции, таймер не запускаем!
    }
...
document.getElementById('save-settings-btn').addEventListener('click', function() {
    ...
    // Если регион изменился, нам нужно сбросить предзагруженную картинку и найти новую!
...
document.getElementById('guess-btn').addEventListener('click', async function() {
    ...
    const playerLatLng = playerMarker.getLatLng().wrap(); // ЗАЩИТА ОТ КЛИКА ПО КЛОНУ КАРТЫ!
...
document.getElementById('home-btn').addEventListener('click', function() {
    ...
    document.getElementById('main-menu').style.display = 'flex'; preloadNext(); // Запускаем поиск для следующей игры
});
